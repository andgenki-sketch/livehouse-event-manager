...
export default function Website(){
 ...
 useEffect(()=>{(async()=>{const {data}=await supabase.from('events').select('id,title,event_date,venue,open_time,start_time,advance_price,door_price,public_description,event_artists(sort_order,booking_status,artists(name,photo_path))').eq('website_published',true).order('event_date');setEvents(data||[])})()},[])
 ...
 return <div className="ts-site">
  ...
  <main className="ts-wrap" id="schedule">
   ...
   <section className="schedule-list">{shown.length?shown.map((e,idx)=>{const d=new Date(e.event_date+'T00:00:00');const artists=(e.event_artists||[]).filter(x=>x.booking_status==='確定'||!x.booking_status).sort((a,b)=>a.sort_order-b.sort_order).map(x=>x.artists?.name).filter(Boolean);return <article className="schedule-row" key={e.id}>
    ...
    <div className="flyer"><div className="flyer-placeholder"><span>Thunder Snake</span><small>ATSUGI</small></div></div>
